  ...
  return (
    ...
    >
      <motion.div
        ref={dockRef}
        className={cn(
          "fixed bottom-4 left-1/2 transform -translate-x-1/2 flex items-end h-14 p-2 gap-3 bg-opacity-90 rounded-xl z-50",
      ...
      <AnimatePresence mode="popLayout">
        {!animatingIndexes.includes(parseInt(id)) ? (
          src ? (
            ...
            <motion.div
              layoutId={id}
              className="relative z-0 w-full h-full rounded-md bg-white/10 border border-black/30 dark:border-white/10 flex items-center justify-center"
            >
              {children}
            </motion.div>
          )
...
export function DockCard({ children, id, onClick }: DockCardProps) {
